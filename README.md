...
- If you use AppImage portable folders feature, you didn't manually integrate search-provider to the system & you want to use the search-provider functionality, than only use portable `appimage-filename.config` and `appimage-filename.cache` folder to make the functionality work.
  - If you want to clean `$HOME` after uninstallation, you need to remove the files below, which are used for search-provider integration:
